...
/**
 * Genera un PDF para una Nota de Débito B
 * @param {PDFDocument} doc - Documento PDF
 * @param {Object} data - Datos de la nota de débito
 */
async function renderNotaDebitoB(doc, data) {
  ...
  const renderPage = async (isOriginal) => {
    ...
    items.forEach((item) => {
      ...
      // Importe alineado a la derecha (con IVA incluido)
    ...
    // Total (en Factura B el IVA está incluido, no se discrimina)
    doc.font("Helvetica-Bold").fontSize(12);
    doc.text("TOTAL:", xTotales, yTotales, { width: 90, align: "right" });
    doc.text(
...
module.exports = renderNotaDebitoB;
