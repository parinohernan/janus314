<script lang="ts">
  ...
  async function cargarDocumentosDeuda(codigoCliente: string) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Error al cargar documentos de deuda');
      }
    
      const data = await response.json();
      console.log("data", data);
      documentosDeuda = data || [];
    } catch (err) {
...
<div class="container mx-auto px-4 py-8">
  <div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
      ...
      {#if clienteSeleccionado}
        <div class="mt-6">
          <h3 class="text-lg font-medium text-gray-900 mb-2">Documentos de Deuda</h3>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  {#each documentosDeuda as doc}
                    <tr>
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ${doc.ImporteTotal.toFixed(2)}
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-red-600">
                        ${doc.SaldoPendiente.toFixed(2)}
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <button 
                          type="button" 
                          ...
                          on:click={() => {
                            // Aquí se podría implementar la lógica para seleccionar este documento
                            // para incluir en el recibo
                          }}
                        >
                          Seleccionar
                        </button>
