<script lang="ts">
  ...
  const loadProveedor = async (): Promise<void> => {
    ...
    try {
      ...
      // Usar el servicio para cargar el proveedor
      const proveedorData = await ProveedorService.obtenerProveedor($page.params.id);
      
      // Si llegamos aquí, se obtuvo el proveedor correctamente
      proveedor = proveedorData as ProveedorCompleto;
      error = null;
    } catch (err: unknown) {
  ...
  onMount(async () => {
    try {
      ...
      // Ejecutar ambas operaciones en paralelo para mejorar rendimiento
      await Promise.all([
        loadCodigosPostales(),
        loadProveedor()
      ]);
  ...
  const handleSubmit = async (event: Event): Promise<void> => {
    ...
    try {
      ...
      const response = await fetchWithAuth(
        isEditing ? `/proveedores/${$page.params.id}` : '/proveedores',
        {
...
<div class="container mx-auto p-4">
  <div class="bg-white p-6 rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold">{isEditing ? 'Editar' : 'Nuevo'} Proveedor</h1>
      <Button variant="secondary" on:click={() => goto('/compras/proveedores')}>
        Volver al listado
      </Button>
    </div>
