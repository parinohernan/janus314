<script lang="ts">
  ...
  const ND_PROVEEDORES_PATH = '/compras/notasdebito';

  onMount(async () => {
    let savedScroll: number | undefined;
    if (browser) {
      const savedState = navigationState.getState(ND_PROVEEDORES_PATH);
  ...
  beforeNavigate(({ from }) => {
    if (from?.url.pathname === ND_PROVEEDORES_PATH && browser) {
      const currentState = navigationState.getState(ND_PROVEEDORES_PATH) || {};
      navigationState.saveState(ND_PROVEEDORES_PATH, {
        ...currentState,
