<script lang="ts">
  ...
  import { Minus, ArrowLeft, DollarSign } from 'lucide-svelte';
...
<div class="space-y-6 pb-8 max-w-2xl mx-auto" transition:fade={{ duration: 200 }}>
  <!-- Header -->
  <div class="bg-gradient-to-r from-red-600 to-rose-600 rounded-xl p-6 text-white shadow-lg">
    <button
    ...
    <div class="flex items-center gap-3">
      <Icon icon={Minus} size={32} strokeWidth={2.5} glass={true} />
      <div>
        <h1 class="text-3xl font-bold mb-1">Registrar Egreso</h1>
        <p class="text-red-100">Caja #{cajaId}</p>
      </div>
    </div>
  </div>
  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <form onsubmit={(e) => { e.preventDefault(); registrarEgreso(); }} class="space-y-6">
      <div>
      ...
      <div class="flex gap-3 pt-4">
        ...
        <button
          type="submit"
          class="flex-1 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={loading}
        >
