...
function _page($$payload, $$props) {
  ...
  head($$payload, ($$payload2) => {
    $$payload2.title = `<title>Gestión de Rubros</title>`;
  });
  $$payload.out += `<div class="container mx-auto p-4"><div class="flex justify-between items-center mb-6"><h1 class="text-2xl font-bold">Gestión de Rubros</h1> `;
  Button($$payload, {
    variant: "primary",
    children: ($$payload2) => {
      $$payload2.out += `<!---->Nuevo Rubro`;
    },
    $$slots: { default: true }
  });
  $$payload.out += `<!----></div> <div class="mb-6 bg-white p-4 rounded-lg shadow-sm"><div class="flex flex-col md:flex-row gap-4"><div class="flex-grow"><label for="search" class="block text-sm font-medium text-gray-700 mb-1">Buscar</label> <div class="relative"><input type="text" id="search" placeholder="Buscar por código o descripción..."${attr("value", filters.search)} class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"> `;
  {
    $$payload.out += "<!--[!-->";
  }
  $$payload.out += `<!--]--></div></div> <div class="w-32"><label for="limit" class="block text-sm font-medium text-gray-700 mb-1">Mostrar</label> <select id="limit" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"><option value="5">5</option><option value="10">10</option><option value="25">25</option><option value="50">50</option></select></div></div></div> `;
  if (loading) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<div class="text-center py-4"><div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div> <p class="mt-2">Cargando rubros...</p></div>`;
  } else if (error) {
    $$payload.out += "<!--[1-->";
    ...
    $$payload.out += "<!--[2-->";
    $$payload.out += `<div class="bg-gray-50 text-center py-10 rounded-lg border border-gray-200"><p class="text-gray-500">No se encontraron rubros con los criterios de búsqueda.</p> `;
    {
      $$payload.out += "<!--[!-->";
    }
    $$payload.out += `<!--]--></div>`;
    ...
    $$payload.out += "<!--[!-->";
    const each_array = ensure_array_like(rubros);
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$payload.out += `<tr class="hover:bg-gray-50"><td class="px-6 py-4 whitespace-nowrap border-b border-gray-200">${escape_html(rubro.Codigo)}</td><td class="px-6 py-4 border-b border-gray-200">${escape_html(rubro.Descripcion || "-")}</td><td class="px-6 py-4 whitespace-nowrap text-right border-b border-gray-200">`;
      Button($$payload, {
        variant: "secondary",
        size: "sm",
        children: ($$payload2) => {
