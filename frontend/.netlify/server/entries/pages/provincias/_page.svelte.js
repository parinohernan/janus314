...
function _page($$payload, $$props) {
  ...
  head($$payload, ($$payload2) => {
    $$payload2.title = `<title>Gestión de Provincias</title>`;
  });
  $$payload.out += `<div class="container mx-auto p-4"><div class="flex justify-between items-center mb-6"><h1 class="text-2xl font-bold">Gestión de Provincias</h1> `;
  Button($$payload, {
    variant: "primary",
    children: ($$payload2) => {
      $$payload2.out += `<!---->Nueva Provincia`;
    },
    $$slots: { default: true }
  });
  $$payload.out += `<!----></div> <div class="mb-6 flex items-center space-x-4"><div class="flex-grow"><input type="text" placeholder="Buscar provincias..." class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"${attr("value", filters.search)}></div> <div><select id="limit" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"><option${attr("value", 5)}>5</option><option${attr("value", 10)}>10</option><option${attr("value", 25)}>25</option><option${attr("value", 50)}>50</option><option${attr("value", 100)}>100</option></select></div></div> `;
  if (loading) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<div class="bg-white p-8 rounded-md shadow text-center"><p class="text-gray-500">Cargando provincias...</p></div>`;
  } else if (error) {
    $$payload.out += "<!--[1-->";
    ...
    $$payload.out += "<!--[2-->";
    $$payload.out += `<div class="bg-white p-8 rounded-md shadow text-center"><p class="text-gray-500">No hay provincias disponibles</p></div>`;
  } else {
    $$payload.out += "<!--[!-->";
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$payload.out += `<tr class="hover:bg-gray-50"><td class="px-6 py-4 whitespace-nowrap border-b border-gray-200">${escape_html(provincia.Codigo)}</td><td class="px-6 py-4 border-b border-gray-200">${escape_html(provincia.Descripcion || "-")}</td><td class="px-6 py-4 whitespace-nowrap text-right border-b border-gray-200">`;
      Button($$payload, {
        variant: "secondary",
        size: "sm",
        children: ($$payload2) => {
